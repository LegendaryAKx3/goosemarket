...
def client():
    ...
    @pytest.fixture
    def mock_supabase():
        """Mock the Supabase client for both prices.py and amm.py."""
    ...
    def test_get_price_valid_poll(client, mock_supabase):
        ...
        assert abs((data["price_yes"] + data["price_no"]) - 1.0) < 0.0001
    ...
    def test_get_price_no_trades(client, mock_supabase):
        ...
        assert data["q_yes"] == 0
        assert data["q_no"] == 0
        assert abs(data["price_yes"] - 0.5) < 0.01
        assert abs(data["price_no"] - 0.5) < 0.01
    ...
    def test_get_price_response_precision(client, mock_supabase):
        ...
        assert response.status_code == 200
        data = response.get_json()
        price_yes_str = str(data["price_yes"])
        if '.' in price_yes_str:
            decimal_places = len(price_yes_str.split('.')[1])
